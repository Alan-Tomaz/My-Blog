...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($idResult) == 1) {
        ...
        if (!$firstname) {
            ...
            $_SESSION["edit-user"] = "Please edit a Avatar";
        } else {
            if ($createPassword !== $confirmPassword) {
                ...
                if ((mysqli_num_rows($userCheckResult) > 0) && ($username !== $idUser["username"] || $email !== $idUser["email"])) {
                    ...
                    $oldAvatarName = $idUser["avatar"];
                    $oldAvatarPath =  "../../img/" . $oldAvatarName;
                    if ($oldAvatarPath) {
                    ...
                    if (in_array($extention, $allowed_files)) {
                        ...
                        if ($avatar["size"] < 1000000) {
                            ...
                            // update user 
                            $query = "UPDATE users SET firstname='$firstname', lastname='$lastname', username = '$username', email = '$email', location = '$location', biography = '$bio', password = '$hashedPassword', avatar = '$avatarName', is_admin='$isAdmin' WHERE id='$id' limit 1";
