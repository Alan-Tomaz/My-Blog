...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($idResult) == 1) {
        ...
        if (!$firstname) {
            $_SESSION["edit-profile"] = "Please Enter a First Name";
        } else if (!$lastname) {
            $_SESSION["edit-profile"] = "Please Enter a Last Name";
        } else if (!$username) {
            $_SESSION["edit-profile"] = "Please Enter a Username";
        } else if (!$email) {
            $_SESSION["edit-profile"] = "Please Enter a Valid E-mail";
        } else if (!$location) {
            $_SESSION["edit-profile"] = "Please Enter a Valid Location";
        } else if (!$bio) {
            $_SESSION["edit-profile"] = "Please Enter a Biography";
        } else if ((strlen($createPassword) < 8 || strlen($confirmPassword) < 8) && (strlen($createPassword) > 0 || strlen($confirmPassword) > 0)) {
            $_SESSION["edit-profile"] = "Please The Password Should Be 8+ Characters";
        } else {

            if ($createPassword !== $confirmPassword) {
                $_SESSION["edit-profile"] = "Passwords Do Not Match";
            } else {
                ...
                if ((mysqli_num_rows($userCheckResult) > 0) && ($id != $userCheck["id"])) {
                    $_SESSION["edit-profile"] = "Username or Email Already Exists";
        ...
        if (isset($_SESSION['edit-profile'])) {
            ...
            header("location: " . ROOT_URL . "admin/pages/edit-profile.php?id=" . $id);
            die();
        } else {
            ...
            if ($passwordIsBlank == true) {
                $query = "UPDATE users SET firstname='$firstname', lastname='$lastname', username = '$username', email = '$email', location = '$location', biography = '$bio', avatar = '$avatarToInsert' WHERE id='$id' limit 1";
            } else {
                $query = "UPDATE users SET firstname='$firstname', lastname='$lastname', username = '$username', email = '$email', location = '$location', biography = '$bio', password = '$hashedPassword', avatar = '$avatarToInsert' WHERE id='$id' limit 1";
            }
        ...
        $_SESSION['edit-profile'] = "This User Doesn't exists";
